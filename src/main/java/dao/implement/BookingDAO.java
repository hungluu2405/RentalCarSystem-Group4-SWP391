...
public class BookingDAO extends DBContext {
    ...
    public int countHistoryBookingsByOwner(int ownerId) {
        String sql = "SELECT COUNT(*) FROM BOOKING B " +
                "JOIN CAR C ON B.CAR_ID = C.CAR_ID " +
                "JOIN USER_PROFILE u ON b.USER_ID = u.USER_ID " +
                "WHERE C.USER_ID = ? AND B.STATUS IN ('Rejected', 'Completed', 'Cancelled')";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, ownerId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return rs.getInt(1);
    ...
    """);
        ...
        // Filter by date range - bookings that overlap with the specified period
    ...
    /**
     * Count current trips for CUSTOMER with simplified filters
     */
    ...
    /**
     * Get history trips for CUSTOMER with simplified filters
     */
    ...
    /**
     * Count history trips for CUSTOMER with simplified filters
     */
...
// ==================== FILTER METHODS FOR CAR OWNER ====================
    ...
    // Helper methods for owner
    public int countCurrentTripsByOwnerId(int ownerId) {
        String sql = """
        SELECT COUNT(*)
        FROM BOOKING b
        JOIN CAR c ON b.CAR_ID = c.CAR_ID
