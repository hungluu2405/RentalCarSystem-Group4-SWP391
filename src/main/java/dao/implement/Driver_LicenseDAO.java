...
public class Driver_LicenseDAO extends DBContext {
    ...
    public Driver_License getLicenseByUserId(int userId) {
        String sql = """
            SELECT d.LICENSE_ID, d.USER_ID, d.LICENSE_NUMBER, d.ISSUE_DATE, d.EXPIRY_DATE, d.IMAGE_URL,
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    dl.setImage_url(rs.getString("IMAGE_URL"));
    ...
    public boolean updateLicense(Driver_License dl) {
        String sql = """
            ...
            SET LICENSE_NUMBER = ?, ISSUE_DATE = ?, EXPIRY_DATE = ?, IMAGE_URL = ?
            WHERE USER_ID = ?
        """;
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            ps.setString(4, dl.getImage_url());
            ps.setInt(5, dl.getUser_id());
